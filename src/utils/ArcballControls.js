...
class ArcballControls extends EventDispatcher {
	...
	makeGizmos = ( tbCenter, tbRadius ) => {
		...
		const curve = new EllipseCurve( 0, 0, tbRadius / 10, tbRadius / 10 );
		const points = curve.getPoints( this._curvePts );
		...
		//material
		const curveMaterialX = new LineBasicMaterial( { color: 0xff8080, fog: false, transparent: true, opacity: 0.6, depthTest: false, depthWrite: false } );
		const curveMaterialY = new LineBasicMaterial( { color: 0x80ff80, fog: false, transparent: true, opacity: 0.6, depthTest: false, depthWrite: false } );
		const curveMaterialZ = new LineBasicMaterial( { color: 0x8080ff, fog: false, transparent: true, opacity: 0.6, depthTest: false, depthWrite: false } );
	 ...
	 */
	scale = ( size, point, scaleGizmos = false ) => {
